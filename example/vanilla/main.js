...
function startUpload() {
  ...
  var options = {
    ...
    onSuccess: function () {
      ...
      anchor.id = "result";
      anchor.textContent = "Status: " + upload.file.name + " (" + upload.file.size + " bytes)";
      var filename = upload.file.name;
      anchor.className = "btn-success";
      uploadList.appendChild(anchor);
      var resourceFileId = upload.url.split('/')[4];
      var source = new EventSource(`https://publish.voodfy.com/v1/resource/${resourceFileId}/status`);
      source.onmessage = function(event) {
        document.getElementById("result").innerHTML = filename + "-" + event.data;
      };
